...
//
// Identity & Legal, Financial, Healthcare and Travel are listed here because
// they have no OCR-signal detector in classifyDocument() and aren't part of
// ASSET_SUBCATEGORIES in scanReceipt.js, so without an entry here they used
// to fall through to the generic label:value scanner (which never matches —
// ID cards print "Name" / "JOHN DOE" on separate lines, not "Name: John
// Doe") and then to the receipt-shaped LLM fallback prompt (name, brand,
// store, amount...), which is why Aadhaar/PAN cards extracted nothing.
// Vehicle/Insurance/Jewellery/Property/Gadgets/Appliances are still handled
// by the asset-style extractor (parseInvoice) in scanReceipt.js and don't
// need an entry here.
const FIELD_LABELS = {
...
// null if this document type isn't in the table above (the caller falls
// back to the generic/asset extractor in that case).
