...
function parseAmount(text) {
  ...
  const unit = match[2];
  if (unit === "k" || unit === "thousand") value *= 1e3; else if (unit === "l" || unit === "lakh" || unit === "lac") value *= 1e5; else if (unit === "cr" || unit === "crore") value *= 1e7;
  return value;
}
...
function findMatchingAssets(query, assets, limit = 5) {
  ...
  const scored = assets.map(asset => {
    const haystack = [ asset.name, asset.brandOrDeveloper, asset.category, asset.subCategory, asset.storeOrSeller, asset.notesOrAddress ].filter(Boolean).join(" ").toLowerCase();
    let score = 0;
    qWords.forEach(w => {
  ...
  return scored.filter(s => s.score > 0).sort((a, b) => b.score - a.score).slice(0, limit).map(s => s.asset);
}
...
function generateReply(message, assets) {
  ...
  switch (intent) {
   ...
   case "warranty_expiring":
    {
      ...
      const upcoming = assets.filter(a => a.expiryDate).map(a => ({
        a: a,
        days: daysBetween(now, new Date(a.expiryDate))
      })).filter(x => x.days >= 0 && x.days <= windowDays).sort((x, y) => x.days - y.days);
   ...
   case "warranty_expired":
    {
      const expired = assets.filter(a => a.expiryDate && new Date(a.expiryDate) < now).sort((a, b) => new Date(b.expiryDate) - new Date(a.expiryDate));
...
function generateAssetSummary(asset) {
  ...
  if (category) overview += ` is registered under your ${category}${asset.subCategory ? ` / ${asset.subCategory}` : ""} vault`; else overview += " is stored in your vault";
  if (asset.issueDate) overview += `, purchased${seller ? ` from ${seller}` : ""} on ${formatDate(asset.issueDate)}`; else if (seller) overview += `, purchased from ${seller}`;
  overview += ".";
...
function generateWarrantyClaimEmail(asset, user) {
  ...
  const refNumber = (asset.invoiceNumber && asset.invoiceNumber !== "-" ? asset.invoiceNumber : null) || (asset.documentNumber && asset.documentNumber !== "-" ? asset.documentNumber : null);
  ...
  bodyParts.push(`Dear Sir/Madam,\n\nI am writing to raise a warranty claim for my ${name}${seller ? `, purchased from ${seller}` : ""}${purchaseDate ? ` on ${purchaseDate}` : ""}. ${warrantyStatusLine}`);
  bodyParts.push(`Item / claim details:\n${detailLines.join("\n")}`);
  bodyParts.push("[Please describe the issue you are facing with the item here — e.g. the defect, when it started, and any troubleshooting already attempted.]");
  bodyParts.push(`I would request you to kindly arrange for a repair, replacement, or servicing of the item at the earliest, in accordance with the applicable warranty terms. The purchase invoice and other supporting documents are available with me and can be shared if required.`);
...
module.exports = {
  ...
  extractWindowDays: extractWindowDays,
  findMatchingAssets: findMatchingAssets,
  detectIntent: detectIntent,
  generateReply: generateReply,
  generateAssetSummary: generateAssetSummary,
  generateWarrantyClaimEmail: generateWarrantyClaimEmail
};