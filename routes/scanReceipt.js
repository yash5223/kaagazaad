...
const OCR_SCRIPT_PATH = path.join(__dirname, '..', 'scripts', 'ocr_engine.py');
const OCR_SUBPROCESS_TIMEOUT_MS = 60 * 1000;
// Runs OCR via a Python subprocess (pytesseract) instead of tesseract.js.
// One subprocess is spawned per request; it recognizes all pages/images for
// that request and returns a single JSON result on stdout. Using execFile
// (not exec/shell) means image paths are passed as discrete argv entries,
// never interpolated into a shell string, so there's no shell-injection risk
// even though the paths are server-generated multer/sharp temp filenames.
async function ocrImageFiles(tessdataDir, imagePaths) {
  const args = [OCR_SCRIPT_PATH, tessdataDir, ...imagePaths];
  const { stdout } = await new Promise((resolve, reject) => {
    execFile(
      PYTHON_BIN,
      args,
      { timeout: OCR_SUBPROCESS_TIMEOUT_MS, maxBuffer: 20 * 1024 * 1024 },
      (err, stdoutData, stderrData) => {
        if (err) {
          const message = stdoutData && stdoutData.trim()
            ? stdoutData.trim()
            : (stderrData && stderrData.trim()) || err.message;
          return reject(new Error(`OCR subprocess failed: ${message}`));
        }
  ...
  try {
    ...
    throw new Error(`OCR subprocess returned invalid JSON: ${parseErr.message}`);
  }
  if (parsed.error) {
    throw new Error(`OCR subprocess error: ${parsed.error}`);
  }
  return {
    text: parsed.text || '',
...
router.post('/scan-receipt', authMiddleware, upload.single('image'), verifyReceiptImage, async (req, res) => {
  ...
  try {
    ...
    if (!rawText && imagesToOcr.length > 0) {
      try {
        const result = await ocrImageFiles(tessdataDir, imagesToOcr);
        rawText = result.text;
        ocrConfidence = result.confidence;
        ocrWords = result.words || [];
      } catch (ocrErr) {
        console.error('[OCR] python subprocess failed:', ocrErr);
        throw ocrErr;
      }
    }
