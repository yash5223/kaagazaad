...
const CFB_EXTENSION_RESOURCE_TYPE = {
  doc: "raw",
  xls: "raw"
};
...
async function verifyUploadedFile(file, allowedExts) {
  ...
  if (restrictSet && !restrictSet.has(claimedExt)) {
    return {
      ok: false,
      reason: `"${claimedExt || "unknown"}" files are not accepted here.`
    };
  }
  ...
  if (!detected) {
    return {
      ok: false,
      reason: "Could not verify the file contents — it may be corrupted or is not a supported type."
    };
  }
  if (detected.mime === CFB_MIME) {
    if (Object.prototype.hasOwnProperty.call(CFB_EXTENSION_RESOURCE_TYPE, claimedExt)) {
      return {
        ok: true,
        resourceType: CFB_EXTENSION_RESOURCE_TYPE[claimedExt],
        ext: claimedExt
      };
    }
    return {
      ok: false,
      reason: "This file looks like a legacy Office document but its extension does not match its contents."
    };
  }
  ...
  if (!rule || !rule.extensions.includes(claimedExt)) {
    return {
      ok: false,
      reason: `The file's actual contents ("${detected.ext || detected.mime}") do not match its ".${claimedExt}" extension.`
    };
  }
  return {
    ok: true,
...
function fileTypeGuard({allowedExts: allowedExts} = {}) {
